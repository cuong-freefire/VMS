...
- `status`: Filter by status ["COMPLETED", "ONGOING"]
...
Get paginated list of events visible to volunteers (active events only).
...
- `start_date_from`: Filter events starting from this date (ISO 8601)
- `start_date_to`: Filter events starting before this date
- `status`: Status filter ["UPCOMING", "ONGOING", "COMPLETED"]
- `has_available_slots`: Boolean, filter events with available slots
...
### UC11: Apply for Event

#### POST /events/:id/apply

Submit application to participate in an event.
...
```json
{
  "motivation": "I am passionate about environmental conservation and want to contribute to keeping our beaches clean.",
  "experience": "Previously participated in 3 beach cleanups."
}
```

**Validation Rules**:

- `motivation`: Required, min 20 chars, max 1000 chars
...
- `400`: Event is full or registration closed
- `404`: Event not found
- `409`: Already applied to this event

---

### UC12: View My Applications

#### GET /applications/my

Get list of current user's event applications.
...
- `event_status`: Filter by event status ["UPCOMING", "ONGOING", "COMPLETED"]
...
**Auth Required**: Yes  
**Role**: VOLUNTEER (own application only)
...
- `400`: Cannot cancel approved/rejected applications
- `403`: Not authorized (not your application)
- `404`: Application not found

---

### UC14: Filter Events by Category
...
### UC48: Submit Event Feedback

#### POST /events/:id/feedback

Submit feedback after participating in an event (only for approved volunteers with attendance).
...
- `comment`: Required, min 10 chars, max 2000 chars
- `suggestions`: Optional, max 1000 chars
...
- `400`: Not eligible to submit feedback (not approved or no attendance)
- `404`: Event not found
- `409`: Feedback already submitted

---

### UC51: View Certificate

#### GET /certificates/my/:event_id

View certificate for completed event participation.
...
- `404`: Certificate not found or not issued yet

---

### UC52: Download Certificate

#### GET /certificates/my/:event_id/download
...
- `Content-Disposition`: `attachment; filename="certificate-{event_id}-{volunteer_id}.pdf"`
...
- `max_volunteers`: Required, min 1, max 1000
...
- `page`: Page number (default: 1)
- `limit`: Items per page (default: 10, max: 50)
- `status`: Filter by status ["PENDING_APPROVAL", "APPROVED", "REJECTED", "UPCOMING", "ONGOING", "COMPLETED"]
    ...
    "items": [
      {
        ...
        "suggestions": "Maybe provide more water stations.",
        "created_at": "2026-07-02T10:00:00Z"
      }
    ],
    "pagination": {
