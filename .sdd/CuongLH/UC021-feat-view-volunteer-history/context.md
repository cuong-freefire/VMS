...
* **Trạng thái bản ghi (Application Status):** Lịch sử bao gồm các trạng thái: `Approved` (Đã duyệt - chờ tham gia), `PENDING` (Chờ duyệt), `APPROVED` (Đã duyệt), `REJECTED` (Bị từ chối), `CANCELLED` (Đã hủy). Schema V3.0 chưa có bảng `attendances` nên chưa hỗ trợ derived status `ATTENDED`.

* **Dữ liệu liên kết:** Mỗi bản ghi lịch sử cần hiển thị đầy đủ thông tin sự kiện tương ứng (tên sự kiện, thời gian diễn ra, địa điểm\). Schema V3.0 chỉ join `application → event`, chưa join `organization`, `attendance`, `certificate`.
...
* **Thống kê tích lũy (Cumulative Metrics):** Hệ thống cần tổng hợp số liệu như: tổng số đơn đăng ký. Các metrics nâng cao (tổng giờ, số hoàn thành) sẽ được bổ sung khi schema có `attendances`.
...
* Giả định rằng danh sách lịch sử sẽ được sắp xếp theo thời gian đăng ký giảm dần (createdAt desc - đơn mới nhất ở đầu danh sách) để người dùng dễ dàng theo dõi các hoạt động gần đây nhất.
...
1. **Phạm vi hiển thị:**
   * **QUYẾT ĐỊNH:** Hiển thị **TẤT CẢ các trạng thái** đơn đăng ký (PENDING, APPROVED, REJECTED, CANCELLED).
   * **Rationale:** Phù hợp với định nghĩa "Lịch sử tình nguyện" tại mục 2, giúp minh bạch hóa hồ sơ và giúp tình nguyện viên biết lý do bị từ chối để cải thiện đơn đăng ký sau này. Schema V3.0 chưa có attendances nên chưa có derived status ATTENDED.

2. **Bảng tóm tắt (Summary Card):**
   * **QUYẾT ĐỊNH:** **CÓ HIỂN THỊ** tổng số đơn đăng ký. Ở Schema V3.0, summary chỉ gồm `total` (tổng số đơn).
   * **Rationale:** Các metrics nâng cao (tổng giờ tích lũy, số hoàn thành) sẽ được bổ sung khi schema có bảng `attendances` và `certificates`.
...
5. **Bộ lọc (Filtering):**
   * **QUYẾT ĐỊNH:** Hỗ trợ lọc theo **Trạng thái** (PENDING, APPROVED, REJECTED, CANCELLED) và **Năm** tham gia. Thêm **tìm kiếm** theo tên sự kiện/địa điểm (client-side).
   * **Rationale:** Giúp người dùng quản lý danh sách hiệu quả. Tìm kiếm được thêm vào vì code đã implement client-side search.

6. **Tìm kiếm (Search):**
   * **QUYẾT ĐỊNH:** **CÓ THỰC HIỆN** chức năng tìm kiếm theo tên sự kiện và địa điểm (client-side filter sau khi query).
   * **Rationale:** Đã được implement trong code hiện tại. Tìm kiếm thực hiện phía client sau khi lấy dữ liệu từ API (filter theo `title` và `location`).
