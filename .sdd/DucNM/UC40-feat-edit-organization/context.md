...
Sau khi tổ chức đã được tạo trong hệ thống, thông tin tổ chức có thể thay đổi theo thời gian (thay đổi tên, địa chỉ, số điện thoại, email, logo). Manager/Admin cần chức năng chỉnh sửa thông tin tổ chức để cập nhật dữ liệu cho chính xác. Ngoài ra, Manager/Admin cũng cần chức năng vô hiệu hóa (soft-delete) tổ chức khi tổ chức đó ngừng hợp tác. Nếu không có chức năng này, dữ liệu tổ chức sẽ nhanh chóng lỗi thời.

## 2. DOMAIN KNOWLEDGE

- **Edit Organization:** Cho phép Manager/Admin cập nhật toàn bộ các trường của tổ chức: name, description, address, contact_phone, contact_email, website, logo.
- **Soft-delete:** Khi Manager/Admin vô hiệu hóa tổ chức (set `is_active = false`), tổ chức không thể được tham chiếu cho sự kiện mới. Tuy nhiên, dữ liệu lịch sử vẫn được giữ nguyên.
...
- **Manager/Admin:** Người có quyền chỉnh sửa và vô hiệu hóa tổ chức.
- **Staff (gián tiếp):** Bị ảnh hưởng khi tổ chức bị vô hiệu hóa — không thể gán tổ chức đó cho sự kiện mới.

## 4. CONSTRAINTS (Ràng buộc cứng)

- **Manager/Admin:** Chỉ Manager và Admin. Mọi role khác bị HTTP 403.
...
- Giả định Manager/Admin có thể sửa một phần hoặc toàn bộ các trường.
