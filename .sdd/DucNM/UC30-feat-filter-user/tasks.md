# Tasks: Filter User (UC30)

**Input**: Design documents từ `.sdd/DucNM/UC30-feat-filter-user/`
...
- UC30 là extension của UC26 — tất cả thay đổi đều trên files đã có
...
**Purpose**: Mở rộng Zod schema để hỗ trợ filter params mới
...
- **User Story 1 (Phase 2)**: Role filter đã có từ UC26 — chỉ thêm tests
...
| T009 + T013 | Service logic — cùng file, sequential |

### Implementation Strategy: MVP First

1. **Phase 1**: Setup (T001) → Zod schema ready
2. **Phase 2**: US1 (T002-T004) → Role filter verified
3. **Phase 3**: US2 (T005-T009) → is_active filter **MVP!**
4. **Phase 4**: US3 (T010-T014) → Multi-filter + date range
5. **Phase 5**: Edge cases + Frontend filter UI (T015-T025)