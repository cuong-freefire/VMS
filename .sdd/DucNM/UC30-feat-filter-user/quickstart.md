...
// NEW UC30: is_active filter
if (is_active !== undefined) {
  where.AND.push({ is_active });
}

// NEW UC30: date range filter
if (from_date) {
...
Cập nhật Swagger JSDoc cho `GET /api/v1/users` — thêm 3 params mới: `is_active`, `from_date`, `to_date`. (Xem contracts/api-get-users-filter.md)

### 4. Tests (`backend/tests/user/user.service.test.js`)

```javascript
// Test cases bổ sung cho UC30:
...
// Thêm handlers:
const handleActiveFilter = (value) => { setIsActive(value); setPage(1); };
const handleFromDateChange = (value) => { setFromDate(value); setPage(1); };
const handleToDateChange = (value) => { setToDate(value); setPage(1); };
