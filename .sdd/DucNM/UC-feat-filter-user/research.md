...
- **Rationale**:
  ...
  - AND logic: tất cả filter params được thêm vào mảng `AND` để kết hợp với search và role filter hiện có.
- **Pattern**:
  ```js
...
- **Rationale**:
  ...
  - Các filter trigger API call ngay khi thay đổi (giống search behavior).
- **Pattern**:
  ```jsx
  // ActiveFilter
