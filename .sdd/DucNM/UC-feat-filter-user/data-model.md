...
| `is_active` | Boolean | No | - | Lọc active/inactive | **Filter User (NEW)** |
| `from_date` | Date (YYYY-MM-DD) | No | - | Ngày tạo từ (inclusive) | **Filter User (NEW)** |
| `to_date` | Date (YYYY-MM-DD) | No | - | Ngày tạo đến (inclusive) | **Filter User (NEW)** |
...
| Field | Type | Role trong Filter |
|-------|------|-------------------|
| `is_active` | Boolean | Lọc theo trạng thái active/inactive |
| `created_at` | DateTime | Lọc theo khoảng thời gian tạo (from_date, to_date) |

**Lưu ý**: Không thay đổi database schema. Các field `is_active` và `created_at` đã tồn tại trong users table.
