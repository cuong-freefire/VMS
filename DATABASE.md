...
VMS sử dụng **21 physical tables** và **1 derived view pattern** (volunteer_history).
...
| **Events** | events, event_categories, event_skills | Member 3, Member 4 |
| **Applications** | applications, application_status_history | Member 3 |
| **Skills** | skills, user_skills, event_skills | Member 4, Member 1, Member 3 |
...
erDiagram
    ...
    skills ||--o{ event_skills : "required by"
    ...
    events ||--o{ event_skills : "requires"
...
**Owner**: Member 3 - TienTD (event requirements), Member 4 - AnhND (skill catalog)  
**Purpose**: Many-to-many: Events ↔ Skills. Supports required skills on Event Detail and skill search/filter for UC09-UC11.  
**Soft Delete**: No (hard delete when unlinking a skill from an event)
...
|--------|------|-------------|-------------|
| id | INT | PRIMARY KEY, AUTO_INCREMENT | Link ID |
| event_id | INT | FOREIGN KEY → events.id | Event requiring the skill |
| skill_id | INT | FOREIGN KEY → skills.id | Required skill |
| created_at | TIMESTAMP | DEFAULT CURRENT_TIMESTAMP | |
...
- 1 event can require many skills.
- 1 skill can be required by many events.
- No duplicate requirement per event: UNIQUE constraint (event_id, skill_id).
- When querying public events, only active skills SHOULD be returned.
...
- Public/discoverable events for Volunteer Event List/Search/Filter are `status = 'PUBLISHED' AND is_active = TRUE`
- Public queries MUST NOT return DRAFT, COMPLETED, CANCELLED, or inactive/soft-deleted events in MVP
- UI-only display status such as UPCOMING/ONGOING MUST be derived from status + start_date + end_date, not stored as the canonical event status
...
| event_id | INT | FOREIGN KEY → events.id | Sự kiện |
| certificate_code | VARCHAR(100) | UNIQUE, NULL | Public certificate code for display/verification |
| status | ENUM('GENERATING', 'AVAILABLE', 'REVOKED', 'UNAVAILABLE') | DEFAULT 'GENERATING', NOT NULL | Certificate lifecycle/download status |
| certificate_url | VARCHAR(500) | NULL | Cloudinary URL or PDF path; required when status = AVAILABLE |
| file_public_id | VARCHAR(255) | NULL | Cloudinary public ID or private storage key |
...
| revoked_at | TIMESTAMP | NULL | Revocation time when status = REVOKED |
...
- Volunteer chỉ được view/download certificate của chính mình
- Chỉ certificate có status = 'AVAILABLE' mới được download
- GENERATING/REVOKED/UNAVAILABLE không được download
- certificate_url hoặc file_public_id MUST exist trước khi chuyển status sang 'AVAILABLE'
- Immutable: KHÔNG được UPDATE certificate_url/file_public_id sau khi status = 'AVAILABLE'; nếu cần thu hồi thì set status = 'REVOKED' và revoked_at
...
| **Member 2 (NamLD)** | feedbacks (submit only), volunteer_history (view) | users, events, event_skills, skills, applications, attendances, certificates, feedbacks | feedbacks (insert only), applications via public ApplicationService contracts |
| **Member 3 (TienTD)** | events, event_skills, applications, application_status_history, attendances, certificates, feedbacks (view only) | users, organizations, event_categories, skills | Own tables + INCREMENT events.approved_participants |
| **Member 4 (AnhND)** | skills, event_categories, user_skills (shared) | users, event_skills | Own tables only |
...
5. **Phase 5: Events**
   ...
   - `013_create_event_skills.sql`

6. **Phase 6: Applications & Workflow**
   - `014_create_applications.sql`
   - `015_create_application_status_history.sql`
   - `016_create_attendances.sql`
   - `017_create_certificates.sql`
   - `018_create_feedbacks.sql`

7. **Phase 7: Notifications & Donations**
   - `019_create_notifications.sql`
   - `020_create_donations.sql`
   - `021_create_payment_transactions.sql`
...
-- Event skill filter (UC09-UC11)
CREATE INDEX idx_event_skills_event ON event_skills(event_id);
CREATE INDEX idx_event_skills_skill ON event_skills(skill_id);

-- Application queries: User's applications by status
CREATE INDEX idx_applications_user_status ON applications(user_id, status);

-- Certificate list/download availability (UC51-UC52)
