...
**Owner**: Member 1 - CuongLH  
**Purpose**: Email verification tokens (UC62) và Password reset OTP (UC07)  
**Soft Delete**: No (delete after verification/reset)
...
| email | VARCHAR(255) | UNIQUE, NOT NULL | Email cần verify hoặc reset |
| otp_hash | VARCHAR(255) | NOT NULL | Bcrypt hash của OTP 6 chữ số |
| type | ENUM('REGISTER', 'RESET_PASSWORD') | DEFAULT 'REGISTER', NOT NULL | Loại OTP (register hay reset password) |
| created_at | TIMESTAMP | DEFAULT CURRENT_TIMESTAMP | Thời gian tạo OTP (TTL 10 phút) |
| last_sent_at | TIMESTAMP | NULL | Thời gian gửi lần cuối (cooldown 60s) |
| attempts | INT | DEFAULT 0, NOT NULL | Số lần nhập sai (max 5) |
| is_locked | BOOLEAN | DEFAULT FALSE, NOT NULL | Đã bị khóa chưa |
| locked_until | TIMESTAMP | NULL | Thời điểm mở khóa (lockout 15 phút) |
...
- OTP expires sau 10 phút (tính từ created_at)
- Cooldown: Chỉ gửi lại OTP sau 60 giây từ last_sent_at
- Lockout: Sau 5 lần nhập sai, khóa 15 phút
- Sau verify/reset thành công: DELETE record
- 1 email chỉ có 1 active OTP cho mỗi type (xóa OTP cũ khi tạo OTP mới)
- OTP PHẢI được hash trước khi lưu vào otp_hash

---

<!-- DEPRECATED: UC07 đã chuyển sang sử dụng bảng email_verifications chung
     với type = 'RESET_PASSWORD' thay vì bảng riêng password_resets.
     Quyết định này được ghi nhận tại UC07 spec.md (1-Table Design).
     Bảng này được giữ lại tạm thời để reference, sẽ xóa sau khi đã migrate data.

#### Table: `password_resets` (DEPRECATED)

**Owner**: Member 1 - CuongLH  
**Purpose**: Password reset tokens (UC07) - DEPRECATED, dùng email_verifications thay thế  
**Soft Delete**: No
...
| reminder_sent_at | TIMESTAMP | NULL | Lần cuối gửi email nhắc nhở (UC65) |
